...
function LoadingTime(seconds, flag){
	showtext($("proceeding_main_txt"), "Fortfahren...");
	$("Loading").style.visibility = "visible";
	
	y = y+progress;
	if(typeof(seconds) == "number" && seconds >= 0){
		if(seconds != 0){
			showtext($("proceeding_main_txt"), "Bitte warten, ");
			showtext($("proceeding_txt"), Math.round(y)+"% Fortfahren");
			--seconds;
			setTimeout("LoadingTime("+seconds+", '"+flag+"');", 1000);
		}
		else{
			showtext($("proceeding_main_txt"), translate("Abgeschlossen!"));
...
function LoadingProgress(seconds){
	...
	if(typeof(seconds) == "number" && seconds >= 0){
		if(seconds != 0){
			...
			$("proceeding_img_text").innerHTML = "Abgeschlossen!";
			y = 0;
