...
function checkPass(o, o1, o2)
{
	...
	alert('Die beiden Kennworter stimmen nicht uberein.');
	return false;
}
				...
				for(var i=0; i < LWFilterList.length; i++){ //validate if the entry is duplicated in list.
					if(document.form.filter_lw_srcip_x_0.value == LWFilterList[i][0] &&
						 ...
						 document.form.filter_lw_dstport_x_0.value == LWFilterList[i][3]){
							alert("Dieser Eintrag war in der Liste vorhanden.");
							return false;
					}
				}
			}
...
{alert('Bitte Wert eingeben zwischen 0 to ' + document.form.qos_maxbw_x_0.value + '.');
...
{alert('Bitte Wert eingeben zwischen 0 to ' + document.form.qos_umaxbw_x_0.value + '.');
...
top.Edit_Flag = "on";
	...
	if(bFlag == 1)
		alert("Felder durfen nicht leer sein!");
	else if(cFlag == 1)
		alert("Diese Tabelle lasst nur " + c + " Eintrage zu!");
	else{	// b == " Del "
		if(s == 'WLFilterList'){
	...
	if(o.value != "")
	{
		if(o.value.length == 12)
		{
			...
			if (!(c>='0'&&c<='9') && !(c>='a'&&c<='f') && !(c>='A'&&c<='F'))
				{alert('Bitte geben Sie die vollstandige MAC-Adresse ein, die aus 12 Hexadezimalzeichen besteht.');
				o.value = "";
				o.focus();
				o.select();
				return false;
...
function validate_string_ssid(o){
	...
	for(var i = 0; i < o.value.length; ++i){
		...
		if(!validate_ssidchar(c)){
			alert("SSID darf das Zeichen nicht enthalten \" "+o.value.charAt(i)+" \".");
			o.value = "";
			o.focus();
			o.select();
			return false;
...
function validate_range(o, min, max) {
	for(i=0; i<o.value.length; i++)
	{
		...
		{
			alert('Bitte Wert eingeben zwischen ' + min + ' bis ' + max);
			//o.value = max;
			o.focus();
			o.select();
			return false;
...
function validate_range_sp(o, min, max) {
	...
	if(o.value<min || o.value>max) {
		alert('Bitte Wert eingeben zwischen ' + min + ' to ' + max + '.');
		o.value = min;
		o.focus();
		o.select();
		return false;
...
{if ( num<0 || num>255 || c!='.')
{alert(o.value + ' ist keine gultige IP-Adresse!');
o.value = "";
o.focus();
o.select();
return false;
...
len = portidx;
if (port>65535)
{alert(port + ' ist keine gultige Portnummer.');
o.value = "";
o.focus();
o.select();
return false;
...
function validate_portrange(o, v){
		...
		for(var i = 0; i < o.value.length; i++){
			...
			if (c>='0'&&c<='9'){
				...
				if (num>65535 || num==0 || (c!=':' && c!='>' && c!='<' && c!=',')){
					alert(o.value + ' ist keine gultige Portnummer.');
					//o.value = "";
					o.focus();
					o.select();
					return false;
		...
		if ((num>65535 && prev!=-3) || (num<1&&prev!=-2) || (prev>num) || (num>=65535&&prev==-2) || (num<=1&&prev==-3)){
			if (num>65535){				
				...
				alert(o.value + ' ist kein gultiger Portbereich!');
				//o.value = "";
				o.focus();
				o.select();
				return false;
...
{if (num>255)
{alert(num + ' ist keine gultige Portnummer.');
o.value = "";
o.focus();
o.select();
return false;
...
add_option(document.form.TriggerKnownApps, "Bitte auswahlen", "User Defined", 1);
...
{alert("Bitte wahlen Sie einen festen Kanal fur WDS.");
...
function onSubmitCtrlOnly(o, s){
	...
	if(s == 'Upload1'){
		if(document.form.file.value){
			...
			alert("Dieses Feld darf nicht leer bleiben!");
...
{alert("Invalid Hostname! The format should be ’xxx.asuscomm.com’ and ’xxx’ cannot start by number.");
return false;
}
...
{alert("Ungültige zeichen eines Domainnamen sind '" + s.charAt(i) +"' !");
return(false);
}
}
return(true);
...
{alert("Felder Lokale IP oder Portbereich durfen nicht leer sein!");
return false;
}
}
...
document.form.action_script.value = "portmapping.sh" + " " + action.value + " " + local.value + " " + port.value;
		...
		if(s == "hostname_check" && document.form.ddns_server_x.value == "WWW.ASUS.COM" ){
			showLoading();
		...
		if ( document.form.ddns_server_x.value == "WWW.ASUS.COM" ){
			...
			document.form.action_mode.value = "Update";
			document.form.action_script.value = "ddnsclient";	/* fake string */
		}
	}
	else{
		document.form.action_mode.value = "Update";
...
function nmode_limitation(){ //Lock add 2009.11.05 for TKIP limitation in n mode.
	if(document.form.wl_gmode.value == "2"){
		if(document.form.wl_auth_mode.selectedIndex == 0 && (document.form.wl_wep_x.selectedIndex == "1" || document.form.wl_wep_x.selectedIndex == "2")){
			alert("Der RT-N12C1 wird die WEP- oder TKIP-Verschlüsselung in AES [n Only] ändern, um sicherzustellen, dass die Clients die richtige Verbindung zur Verfügung gestellt bekommen.");
	...
	if(document.form.wl_auth_mode.value != "shared" && document.form.wl_auth_mode.value != "open"){
...
{if (o.value == "DROP")
alert("Die Auswahl VERWERFEN verwirft samtliche WAN-zu-LAN-Pakete mit Ausnahme der Treffer aus der Filtertabelle. Bitte setzen Sie diese Funktion mit Umsicht ein.");
}
else if (s == "FirewallConfig" && v=="LanWanDefaultAct")
{if (o.value == "DROP")
alert("Die Auswahl VERWERFEN verwirft samtliche LAN-zu-WAN-Pakete mit Ausnahme der Treffer aus der Filtertabelle. Bitte setzen Sie diese Funktion mit Umsicht ein.");
}
...
{change_ddns_setting(o.value);
		...
		if(o.value == "off"){
				document.form.wl_wme_no_ack.options.seletedIndex=0;
				inputCtrl(document.form.wl_wme_no_ack, 0);
			}
		else{
				if(document.form.wl_gmode.value == "1"){ // b/g mode
...
document.getElementsByTagName("a")[0].setAttribute("href","http://192.168.1.1/index.asp");
showhide("link", 0);
...
{rst = confirm("Wenn Sie eindeutiges AES aktivieren mochten, sollte die Authentisierung auf Offenes System eingestellt sein!");
if (rst)
...
{rst = confirm("Wenn Sie eindeutiges AES aktivieren mochten, sollte auch WEP aktiviert sein!");
if (rst)
...
function valid_WPAPSK(o){
	if(o.value.length >= 64){
		...
		alert("Der gemeinsam genutzte Schlussel (Pre-shared key) kurzer als 64 Zeichen sein!");
		return false;
	}
...
function checkSubnet(){
	...
	if(!matchSubnet(document.form.lan_ipaddr.value, document.form.dhcp_start.value, 3) ||
		...
		if(confirm("Die Einstellungen des DHCP-Servers stimmen nicht mit der aktuellen IP-Adresse im LAN uberein. Mochten Sie dies automatisch andern lassen?")){
...
{if (confirm("Die Einstellungen des DHCP-Servers stimmen nicht mit der aktuellen IP-Adresse der Wireless-Firewall uberein. Mochten Sie dies automatisch andern lassen?"))
...
tourl = "https://controlpanel.tzo.com/cgi-bin/tzopanel.cgi"
...
// check the system's password
// if you click this input, it will alert to allow you to input 16 characters.
function chk_password_len(o){
	if (o.value.length < 17)
		alert("Passwort erlaubt nur die Eingabe 16 Zeichen!");
	return true;
}
